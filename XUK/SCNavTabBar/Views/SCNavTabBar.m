...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        [button setTitleColor:NOMALTITLECOLOR forState:UIControlStateNormal];
        [button addTarget:self action:@selector(itemPressed:) forControlEvents:UIControlEventTouchUpInside];
        [button setBackgroundColor:NOMALBACKCOLOR];
        [_navgationTabBar addSubview:button];
        if (0 == index) {
            [button setBackgroundColor:SELECTBACKCOLOR];
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    if (_currentItemIndex != currentItemIndex) {
        ...
        [oldButton setBackgroundColor:NOMALBACKCOLOR];
        [oldButton setTitleColor:NOMALTITLECOLOR forState:UIControlStateNormal];
